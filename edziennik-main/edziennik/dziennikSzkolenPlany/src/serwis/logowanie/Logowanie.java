...
public class Logowanie implements Serializable{
	...
	public String logowanieValidate() {
		...
		if(userLogin.size()==1)
			...
			{
				if(userLogin.get(0).getPoziomUprawnien().equals("1") || userLogin.get(0).getPoziomUprawnien().equals("4") || userLogin.get(0).getPoziomUprawnien().equals("5") || userLogin.get(0).getPoziomUprawnien().equals("6"))
				{	
						idUsers=userLogin.get(0).getIdUsers();
						session.setAttribute("idUser", idUsers);
						session.setAttribute("zaklad", userLogin.get(0).getZakladToString());
						session.setAttribute("userImieNazwisko", userLogin.get(0).getImieUsers()+" "+userLogin.get(0).getNazwiskoUsers());
						...
						usmanabean.create(rl.zapiszLogiOpis(dataFormGlownyTimeStamp, "logowanie","plany",sessionLoged,userZalogowany.getIdUsers(),userZalogowany.getNazwiskoUsers()+" "+userZalogowany.getImieUsers(), userZalogowany.getIdUsers(),"zalogowano", userZalogowany.getStatusUsers(),opis,userZalogowany.getIdUsers()));
						rl=null;
						
						return"/start/start.xhtml?faces-redirect=true";
				}
			}else {
	...
	public void logoutStart() {
		if(sessionLoged !=null) {
			...
			crudAll.create(rl.zapiszLogiOpis(dataFormGlownyTimeStamp, "wylogowanie","plany",sessionLoged,userZalogowany.getIdUsers(),userZalogowany.getNazwiskoUsers()+" "+userZalogowany.getImieUsers(), userZalogowany.getIdUsers(),"wylogowano", userZalogowany.getStatusUsers(),opis,userZalogowany.getIdUsers()));
			rl=null;
