...
public class terminarz implements Serializable{
    ...
    public void init() {
		...
		hms.put("ktoWpisal", zalogowanyUser.getIdUsers());
		hms.put("zaklad", zalogowanyUser.getZakladUser());
		
		if(Integer.parseInt(zalogowanyUser.getPoziomUprawnien())<7) {
			hms.put("idPluton", "%");
		}else
    ...
    public void addEvent(ActionEvent actionEvent) {
 		...
 		if(event.getId() == null) {
        	...
        	terminarzEvent.setDoWiadomosci(terminarzEvent.getDoWiadomosci());
 			terminarzEvent.setKtoWpisal(zalogowanyUser.getIdUsers());
 		...
 		current.executeScript("PF('eventDialog').hide()");
        	...
        	{ if(edycja) {        	
		        	 if(zalogowanyUser.getIdUsers()==terminarzEvent.getKtoWpisal() || zalogowanyUser.getPoziomUprawnien().equals("1")) {
				        	terminarzEvent.setTerminOd(event.getStartDate());
				        	terminarzEvent.setTerminDo(event.getEndDate());
				        	terminarzEvent.setTresc(event.getTitle());
				        	terminarzEvent.setCalyDzien(event.isAllDay());
    ...
    public void onEventMove(ScheduleEntryMoveEvent event) {
       if(edycja) {
    	 ...
    	 if(zalogowanyUser.getIdUsers()==terminarzEvent.getKtoWpisal()) {
       ...
       /*
    		Date datStart = ((DefaultScheduleEvent) event.getScheduleEvent()).getStartDate();
    		Date datEnd=((DefaultScheduleEvent) event.getScheduleEvent()).getEndDate();
    		Calendar cal=Calendar.getInstance();
    		cal.setTime(datStart);
    		cal.set(Calendar.HOUR, 0);
    		cal.set(Calendar.MINUTE, 0);
    		((DefaultScheduleEvent) event.getScheduleEvent()).setStartDate(cal.getTime());
    		cal.setTime(datEnd);
    		cal.set(Calendar.HOUR, 0);
    		cal.set(Calendar.MINUTE, 0);
    		((DefaultScheduleEvent) event.getScheduleEvent()).setEndDate(cal.getTime());
    		*/
